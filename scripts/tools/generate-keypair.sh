...
openssl genpkey -algorithm RSA -out "$PRIVATE_KEY_FILE" -pkcs8 -aes256 \
    -pass pass:YOUR_PRIVATE_KEY_PASSPHRASE
...
openssl rsa -pubout -in "$PRIVATE_KEY_FILE" -out "$PUBLIC_KEY_FILE" \
    -passin pass:YOUR_PRIVATE_KEY_PASSPHRASE
...
echo ""
echo "🔍 验证密钥对..."
echo "test-data" | openssl rsautl -sign -inkey "$PRIVATE_KEY_FILE" -passin pass:YOUR_PRIVATE_KEY_PASSPHRASE | \
