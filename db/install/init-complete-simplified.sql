...
-- 版本: 2.1 (简化版)
-- 创建日期: 2025-10-10
-- 说明: 这是一个精简的数据库初始化脚本，移除了未使用的表
--       只保留实际使用的功能模块
-- ============================================================
-- 相比完整版的改进:
-- ✅ 移除了 applications 表（使用 hpc_applications 替代）
-- ✅ 移除了 application_categories, application_tags, application_versions
-- ✅ 保留了所有核心功能和权限系统
-- ============================================================
...
-- ============================================================
-- 第三部分: HPC 应用程序管理系统（简化版）
-- ============================================================

-- 3.1 HPC应用程序规范表（主要使用的表）
...
CREATE INDEX IF NOT EXISTS idx_hpc_applications_name ON hpc_applications ((metadata->>'name'));
CREATE INDEX IF NOT EXISTS idx_hpc_applications_category ON hpc_applications ((metadata->>'category'));
...
VALUES
('欢迎使用HPC管理平台', '欢迎使用高性能计算管理平台！本平台使用新一代应用规范系统。', 'success', 10, true, 'admin'),
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'HPC 管理平台数据库初始化完成！(简化版)';
  RAISE NOTICE '========================================';
  RAISE NOTICE '';
  RAISE NOTICE '✓ 核心用户系统 (users, user_groups, user_group_memberships)';
  RAISE NOTICE '✓ 作业管理系统 (jobs, job_status_history)';
  RAISE NOTICE '✓ HPC应用系统 (hpc_applications, hpc_application_usage) - 简化版';
  RAISE NOTICE '✓ 权限管理系统 (file_permissions, webshell_permissions, clipboard_permissions + 日志表)';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '相比完整版的简化:';
  RAISE NOTICE '  ✗ 移除了 applications 表（使用 hpc_applications 替代）';
  RAISE NOTICE '  ✗ 移除了 application_categories, application_tags, application_versions';
  RAISE NOTICE '  ✗ 移除了 view_jobs 表（VNC 功能通过 Slurm 实时查询实现）';
  RAISE NOTICE '  ✓ 保留了所有核心功能和真正使用的表';
  RAISE NOTICE '';
  RAISE NOTICE '默认管理员账户:';
  RAISE NOTICE '  用户名: admin';
  RAISE NOTICE '  邮箱: admin@hpc-platform.local';
  RAISE NOTICE '';
  RAISE NOTICE '总表数: ~19 个（移除未使用的表）';
  RAISE NOTICE '========================================';
END $$;
