...
在代码中发现多处直接硬编码IP地址 `192.168.31.130` 的问题，这违反了配置管理的最佳实践，导致：
...
```typescript
// 生成VNC URL
const nodeIp = '192.168.31.130' // 使用节点IP
const vncUrl = `http://192.168.31.130:6080/vnc.html?host=${nodeIp}&port=${job.vncPort}`
```
...
```typescript
const nodeIp = '192.168.31.130' // 使用节点IP
job.vncUrl = `http://192.168.31.130:6080/vnc.html?host=${nodeIp}&port=${job.vncPort}`
```
...
**修复前**:
```javascript
const vncUrl = `http://192.168.31.130:6080/vnc.html?host=${nodes[0]}&port=${port}`;
vncUrl: nodes.length > 0 ? `http://192.168.31.130:6080/vnc.html?host=${nodes[0]}&port=${port}` : undefined
```
...
- `NOVNC_GATEWAY`: noVNC网关地址（默认: 192.168.31.130）
- `NOVNC_PORT`: noVNC网关端口（默认: 6080）
- `DEFAULT_NODE_IP`: 默认节点IP地址（默认: 192.168.31.130）

### 2. 环境变量设置示例

```bash
# .env.local 或 .env 文件
NOVNC_GATEWAY=192.168.31.130
NOVNC_PORT=6080
...
// ❌ 避免模式
const vncUrl = `http://192.168.31.130:6080/vnc.html?host=192.168.31.130&port=${port}`
```
