...
# 检查 Supabase 连接
psql "postgresql://postgres:YOUR_DB_PASSWORD@192.168.1.10:5432/postgres" -c "SELECT 1;"
...
# 2. 检查数据库连接
psql "postgresql://postgres:YOUR_DB_PASSWORD@192.168.1.10:5432/postgres" -c "SELECT COUNT(*) FROM active_notifications;"
...
const token = jwt.sign({username: 'test'}, process.env.JWT_SECRET || 'your-jwt-secret-here', {expiresIn: '7d'});
...
```bash
# 1. 备份数据库
pg_dump -h 192.168.1.10 -U postgres postgres > notification_backup.sql
