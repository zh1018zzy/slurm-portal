...
```
WARNING: compute-node-01:101 is taken because of /tmp/.X101-lock
Remove this file if there is no X server compute-node-01:101
...
```typescript
// 检查compute-node-01节点上实际占用的display号
async function checkVtdevOccupiedDisplays(): Promise<number[]> {
  // 1. 检查X锁文件
  const { stdout: lockFiles } = await execFileAsync('ssh', ['compute-node-01', 'ls', '/tmp/.X*-lock'])
  
  // 2. 检查X11 socket文件
  const { stdout: socketFiles } = await execFileAsync('ssh', ['compute-node-01', 'ls', '/tmp/.X11-unix/X*'])
  
  // 3. 检查VNC端口占用
  const { stdout: vncPorts } = await execFileAsync('ssh', ['compute-node-01', 'netstat', '-tlnp | grep ":59"'])
