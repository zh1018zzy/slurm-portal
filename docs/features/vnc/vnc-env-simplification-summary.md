...
### 2. 保留的环境变量
- **DEFAULT_VNC_NODE_IP**: VNC节点的IP地址（如：192.168.31.130）
...
#### lib/vnc-manager.ts
- `checkVtdevOccupiedDisplays()` 函数：使用 `DEFAULT_VNC_NODE_IP` 替代硬编码的 `vtdev` 主机名
- SSH连接直接使用IP地址而不是主机名
...
**修改内容**：
- 移除硬编码的 `192.168.31.130`，使用 `localhost` 作为默认值
- 更新测试逻辑，移除对 `vtdev` 主机名的依赖
...
作业ID | 节点 | 生成的VNC URL
-------|------|---------------
105 | compute01 | http://localhost:6080/vnc.html?host=192.168.31.130&port=6001
107 | compute02 | http://localhost:6080/vnc.html?host=192.168.31.130&port=6003
```
