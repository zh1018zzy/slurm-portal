...
function getNodeIpMap(): Record<string, string> {
  ...
  // 默认映射
  return {
    'vtdev': '192.168.31.130',  // vtdev节点的IP地址
    'haomgt': '192.168.31.100', // haomgt节点的IP地址
  }
}
```
...
```bash
# 动态获取节点IP地址
if [ "$HOSTNAME" = "vtdev" ]; then
  NODE_IP="192.168.31.130"
elif [ "$HOSTNAME" = "haomgt" ]; then
...
# 或者在 .env 文件中
NODE_IP_MAP={"vtdev":"192.168.31.130","haomgt":"192.168.31.100"}
```

### 支持的节点

当前支持的节点映射：
- `vtdev` → `192.168.31.130`
- `haomgt` → `192.168.31.100`
...
```bash
# 测试生成的VNC URL
# 修复前: http://192.168.31.130:6080/vnc.html?host=vtdev&port=6001
# 修复后: http://192.168.31.130:6080/vnc.html?host=192.168.31.130&port=6001
```
