...
const gatewayIp = process.env.NOVNC_GATEWAY || '192.168.31.130'
const nodeIp = process.env.DEFAULT_NODE_IP || '192.168.31.130'
...
const vncNode = process.env.VNC_NODE || 'vtdev'
const gatewayIp = process.env.NOVNC_GATEWAY || '192.168.31.130'
const gatewayPort = process.env.NOVNC_PORT || '6080'

// 方法1: 尝试从Slurm获取实时节点信息
let nodeIp = process.env.DEFAULT_NODE_IP || '192.168.31.130'
...
try {
  ...
  if (jobInfo.nodes && jobInfo.nodes.length > 0) {
    ...
    // 如果第一个节点是vtdev或VNC_NODE，使用DEFAULT_NODE_IP
    if (firstNode === 'vtdev' || firstNode === vncNode) {
      nodeIp = process.env.DEFAULT_NODE_IP || '192.168.31.130'
    } else {
      // 否则使用节点名（需要确保DNS解析）
      nodeIp = firstNode
    }
...
-------|------|-----------|-----------|------
105 | vtdev | http://10.0.0.201:6080/vnc.html?host=192.168.31.130&port=6004 | http://10.0.0.201:6080/vnc.html?host=192.168.31.130&port=6004 | ❌ 未修复
106 | compute01 | http://10.0.0.201:6080/vnc.html?host=192.168.31.130&port=6005 | http://10.0.0.201:6080/vnc.html?host=compute01&port=6005 | ✅ 已修复
107 | vtdev,compute02 | http://10.0.0.201:6080/vnc.html?host=192.168.31.130&port=6006 | http://10.0.0.201:6080/vnc.html?host=192.168.31.130&port=6006 | ❌ 未修复
```

### 说明
- 作业106（compute01节点）的host参数已正确修复
- 作业105和107（vtdev节点）保持不变，因为vtdev节点确实应该使用`DEFAULT_NODE_IP`
...
# VNC节点默认IP（用于vtdev节点）
DEFAULT_NODE_IP=192.168.31.130
...
1. **设置VNC_NODE环境变量**为VNC服务器节点名（如vtdev）
