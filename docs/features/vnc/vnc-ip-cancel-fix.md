...
用户报告了两个问题：
1. **VNC连接URL中使用localhost而不是IP地址** - `http://192.168.31.130:6080/vnc.html?host=localhost&port=6003`
...
### 1. **VNC URL使用IP地址**
- ✅ VNC连接URL现在使用正确的IP地址：`http://192.168.31.130:6080/vnc.html?host=192.168.31.130&port=6003`
...
# 验证返回的URL格式
# 期望: http://192.168.31.130:6080/vnc.html?host=192.168.31.130&port=6003
# 而不是: http://192.168.31.130:6080/vnc.html?host=localhost&port=6003
```
...
```bash
# VNC节点IP地址
DEFAULT_VNC_NODE_IP=192.168.31.130
...
# 节点IP映射（可选，JSON格式）
NODE_IP_MAP='{"haomgt":"192.168.31.130","localhost":"192.168.31.130","*":"192.168.31.130"}'
```
