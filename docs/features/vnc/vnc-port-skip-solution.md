...
```bash
# 环境变量
export DEFAULT_VNC_NODE_IP="compute-node-01"           # VNC节点IP
...
```bash
# 检查特定端口
ssh compute-node-01 "netstat -tlnp | grep :5901"

# 检查进程信息
ssh compute-node-01 "ps -p $(netstat -tlnp | grep :5901 | awk '{print \$7}' | cut -d'/' -f1) -o pid,ppid,cmd"
```
