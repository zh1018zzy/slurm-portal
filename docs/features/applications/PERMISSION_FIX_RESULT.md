...
### 问题描述
用户demo_user登录后只能看到6个应用，而不是预期的14个应用。控制台显示：
```
[HPC Application Center] Loaded 6 applications for user demo_user
```
...
# 带用户过滤
curl 'http://localhost:3000/api/applications?forUser=demo_user' | jq '.data | length'
# 修复前: 6
# 修复后: 9
```
...
### 测试用例1: API返回正确数量
```bash
curl -s 'http://localhost:3000/api/applications?forUser=demo_user' | jq '.total'
# 预期: 9
# 实际: 9 ✅
```
...
### 测试用例2: AI应用正确返回
```bash
curl -s 'http://localhost:3000/api/applications?forUser=demo_user' | \
...
// 在应用的 visibility.allowedUsers 中添加用户名
{
  "visibility": {
    "isPublic": false,
    "allowedUsers": ["demo_user", "bio_user1"],  // 只有这些用户能看到
    "allowedGroups": [],
    "allowedDepartments": []
  }
}
...
- ✅ 用户demo_user现在可以看到9个应用（包括3个AI应用）
