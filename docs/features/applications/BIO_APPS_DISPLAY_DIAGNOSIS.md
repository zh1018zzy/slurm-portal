...
### 1. API返回数据
```bash
curl 'http://localhost:3000/api/applications?forUser=demo_user'
```
...
查看浏览器Console:
```
[HPC Application Center] Loaded X applications for user demo_user
```
...
### 步骤1: 验证API数据
```bash
curl -s 'http://localhost:3000/api/applications?forUser=demo_user' | jq '{
  total: .total,
...
查看加载日志:
```
[HPC Application Center] Loaded 11 applications for user demo_user
```
