...
# 示例: /lustre/home/demo_user
```

### 3. NFS集群
```bash
# 用户主目录: /nfs/home/${username}
# 示例: /nfs/home/demo_user
```

### 4. 自定义路径
```bash
# 用户主目录: /data/users/${username}
# 示例: /data/users/demo_user
```

### 5. 云环境
```bash
# 用户主目录: /mnt/efs/users/${username}
# 示例: /mnt/efs/users/demo_user
```
...
// 获取用户主目录
const home = await UserHomeManager.getUserHome('demo_user')
console.log(home) // 输出: /home/demo_user 或其他路径

// 获取用户信息
const userInfo = await UserHomeManager.getUserInfo('demo_user')
console.log(userInfo) // 输出: { home: '/home/demo_user', uid: 2000, gid: 2000, exists: true }

// 创建作业目录
const jobDirInfo = await UserHomeManager.createJobDirectory('demo_user')
console.log(jobDirInfo.jobDir) // 输出: /home/demo_user/my-jobs/job_1234567890_abcdef
```
...
```typescript
// 清除特定用户缓存
UserHomeManager.clearCache('demo_user')
